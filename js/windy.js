...
var Windy = function (params, current_ID) {
	...
	var buildGrid = function (data, callback) {
		...
		function interpolate(λ, φ) {
			...
			if (!STOPEVOLVING) {
				...
				var fi = Math.floor(i),
				ci = fi + 1; // floor i, ceiling i
				var fj = Math.floor(j),
				cj = fj + 1; // floor j, ceiling j
  ...
  var generateColorGradient = function(startColor, endColor) {
	...
	var fadeOut = function () {
		...
		var fadeFuncVar = setInterval(fadeOnce, 100);

        function fadeParticlesOnce() {

            PARTICLE_LINE_REDUCTION = .05;
            if ( PARTICLE_LINE_WIDTH - PARTICLE_LINE_REDUCTION > 0 ) {
                PARTICLE_LINE_WIDTH -= PARTICLE_LINE_REDUCTION;
            } else {
                clearInterval( fadeParticleFuncVar );
            }

        }

        var fadeParticleFuncVar = setInterval(fadeParticlesOnce, 100);
